...
class Perceptron:
	...
	def __init__(self, amostras, saidas, taxa_aprendizado=0.1, epocas=10000, limiar=-1):
	...
	def treinar(self):
		...
		for i in range(self.num_amostra):
			self.pesos.append(random.randint(0,1))
		...
		while True:
			...
			num_epocas += 1
			#print('Em época número ', str(num_epocas), 'amostra é ', self.amostras)
	...
	def testar(self, amostra, classe1, classe2):
		...
		y = self.sinal(u)
		if y == -1:
			print('A amostra ' + str(amostra) +' pertence a classe %s' % classe1 + ' com peso de ' + str(u))
		else:
			print('A amostra ' + str(amostra) +' pertence a classe %s' % classe2 + ' com peso de ' + str(u))
...
rede = Perceptron(amostras=amostras, saidas=saidas, taxa_aprendizado=0.1, epocas=10000)

# treina a rede
rede.treinar()

# testando a rede
print('Vetor com as amostras', amostras, '\n')
print('Vetor de pesos', rede.pesos, '\n')
for teste in testes:
